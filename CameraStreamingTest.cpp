...
struct FrameListener : public ConsumerBase::FrameAvailableListener {
	...
	virtual void onFrameAvailable(const BufferItem& /* item */) {
		...
		mLastTime = mCurrentTime;
		ALOGD("onFrameAvailable::duration = %d", mDuration);
...
int main(int argc, char **argv) {
	...
	for (int32_t i = 0; i < numCameras; i++) {
		...
		/* 
		String8 consumerName = String8::format("ImageReader-%dx%df%xm%d-%d", width, height, format, maxImages, getpid());
		*/
		...
		gbConsumer->setDefaultBufferFormat(HAL_PIXEL_FORMAT_YV12);
